...
:doc:`??? <ch-relationships>` explains the technical details.
...
for the upload (see :ref:`s-f-Urgency`). It is not possible to
...
``Closes`` field in the ``.changes`` file (see :ref:`s-f-Closes`).
...
in the ``.changes`` file (see :ref:`s-f-Changed-By`), and then later
...
further details). Also see `??? <#s-pkgcopyright` for further
