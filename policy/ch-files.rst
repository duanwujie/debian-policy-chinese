...
:ref:`s-maintscripts` and `??? <#s-conflicts` respectively.) If
this case happens, one of the programs must be renamed. The maintainers
should report this to the ``debian-devel`` mailing list and try to find
a consensus about which program will have to be renamed. If a consensus
...
``DEB_BUILD_OPTIONS`` (see :ref:`s-debianrules-options`). This
variable can contain several flags to change how a package is compiled
and built.
...
This section has moved to :doc:`??? <ch-sharedlibs>`.
...
:ref:`s-writing-init` for more information about writing ``init.d``
scripts.
...
:doc:`??? <ch-maintainerscripts>` for more information.) These scripts
must be idempotent (i.e., must work correctly if ``dpkg`` needs to
...
upgrades), and must otherwise be good citizens.
...
is called with the argument ``purge`` (see :ref:`s-removedetails`).
